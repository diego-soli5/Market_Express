...
async function getPaginatedPage(pgnum) {
    ...
    const url = `/Admin/Binnacle/Movement/GetTable?${params}&PageNumber=${pgnum}`;
    ...
    try {
        ...
        document.querySelector("#divMovementTable").innerHTML = text;
...
btnReport.addEventListener("click", function (e) {
    window.open(`/Admin/Binnacle/MovementReport?${params}`, '_blank');
});
    ...
    $("#usernametextbox").autocomplete({
        source: function (request, response) {
            $.ajax({
                ...
                dataType: "json",
                data: { query: $("#usernametextbox").val(), onlyAdmin: true },
